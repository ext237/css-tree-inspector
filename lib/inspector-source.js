(function (root) {
  ...
  function inspectedPageTask(mode, options) {
    const VERSION = "1.6.0";
    const includeAllAttributes = Boolean(options && options.includeAllAttributes);
    const combineStyles = Boolean(options && options.combineStyles);
    const recoveredStyleSheets = new Map(
      Array.isArray(options && options.recoveredStyleSheets)
        ? options.recoveredStyleSheets.map((sheet) => [new URL(sheet.url, location.href).href, sheet.content])
        : []
    );
    ...
    const VISUAL_ATTRIBUTES = new Set([
      "alt", "checked", "colspan", "dir", "disabled", "height", "hidden", "href",
      "inert", "lang", "multiple", "name", "open", "placeholder", "popover", "readonly",
      "required", "role", "rowspan", "selected", "sizes", "span", "src", "srcset", "style",
      "type", "width", "aria-checked", "aria-current", "aria-disabled", "aria-expanded",
      "aria-hidden", "aria-pressed", "aria-selected"
    ]);
    ...
    function collectStyleRules() {
      ...
      function recoveredRules(sheet) {
        if (!sheet || !sheet.href) return null;
        const url = new URL(sheet.href, location.href).href;
        if (!recoveredStyleSheets.has(url)) return null;
        const parserDocument = document.implementation.createHTMLDocument("");
        const style = parserDocument.createElement("style");
        style.textContent = recoveredStyleSheets.get(url);
      ...
      for (const sheet of sheets) {
        ...
        try { visit(sheet.cssRules, sheetContext, sheetConditions, sheetActive); }
          ...
          try {
            const rules = recoveredRules(sheet);
            if (!rules) throw error;
            visit(rules, sheetContext, sheetConditions, sheetActive);
          } catch (recoveryError) {
            recordAccessIssue(sheet, recoveryError);
          }
        }
      }
        ...
        for (const selector of rule.selectors) {
          ...
          records.push({
            ...
            data: {
              ...
              ...(rule.atRuleContext.length ? { atRuleContext: rule.atRuleContext } : {}),
    ...
    function pseudoData(element, pseudo) {
      ...
      const state = stateCSS(element, pseudo);
      const conditional = conditionalCSS(element, pseudo);
      const computed = relevantComputedStyle(element, pseudo);
      return {
        ...(Object.keys(computed).length ? { computedCSS: computed } : {}),
        ...(state.length ? { stateCSS: state } : {}),
        ...(conditional.length ? { conditionalCSS: conditional } : {})
      };
    }

    function pseudoElementsData(element) {
      const pseudoElements = {};
      const before = pseudoData(element, "::before");
      const after = pseudoData(element, "::after");
      if (before) pseudoElements.before = before;
      if (after) pseudoElements.after = after;
      return Object.keys(pseudoElements).length ? pseudoElements : null;
    }

    function safeAttributes(element) {
      const excluded = new Set(["id", "class"]);
      const attributes = {};
      for (const attribute of element.attributes) {
        const name = attribute.name.toLowerCase();
        if (excluded.has(name)) continue;
        if (!includeAllAttributes && !VISUAL_ATTRIBUTES.has(name)) continue;
    ...
    function isInspectableElement(element) {
      return !NON_RENDERING_ELEMENTS.has(element.localName.toLowerCase());
    }

    function preservesWhitespace(element) {
      const tagName = element.localName.toLowerCase();
      if (tagName === "pre" || tagName === "textarea") return true;
      const style = getComputedStyle(element);
      return ["pre", "pre-wrap", "pre-line", "break-spaces"].includes(style.whiteSpace)
        || ["preserve", "preserve-breaks", "break-spaces"].includes(style.whiteSpaceCollapse);
    }

    function cleanTextNode(node, preserve) {
      const value = node.nodeValue || "";
      if (preserve) return value;
      let cleaned = value
        .replace(/^(?:[ \t]*\r?\n)+[ \t]*/, "")
        .replace(/[ \t]*(?:\r?\n[ \t]*)+$/, "");
      if (!node.previousSibling) cleaned = cleaned.replace(/^[ \t]+/, "");
      if (!node.nextSibling) cleaned = cleaned.replace(/[ \t]+$/, "");
      return /\S/.test(cleaned) ? cleaned : "";
    }

    function directTextNodes(element) {
      if (["input", "select", "option"].includes(element.localName.toLowerCase())) return [];
      const output = [];
      const preserve = preservesWhitespace(element);
      for (const node of element.childNodes) {
        if (node.nodeType !== Node.TEXT_NODE) continue;
        const text = cleanTextNode(node, preserve);
        if (!text && !preserve) continue;
        output.push({ text });
      }
      return output;
    }
    ...
    function inspectElement(element, rootElement) {
      const children = [];
      const pseudoElements = pseudoElementsData(element);
      const state = stateCSS(element);
      const conditional = conditionalCSS(element);
      const textNodes = directTextNodes(element);
      const attributes = safeAttributes(element);
      const computed = relevantComputedStyle(element);
      for (const child of element.children) {
        if (isInspectableElement(child)) children.push(inspectElement(child, rootElement));
      }
      return {
        ...
        ...(element.shadowRoot ? { shadowRoot: { supported: false, mode: element.shadowRoot.mode || "open" } } : {}),
        ...(children.length ? { subTreeElements: children } : {})
      };
    }

    function combineStyleDefinitions(rootElement) {
      const fields = [
        ["computedCSS", "computedCSSRef", "computed"],
        ["stateCSS", "stateCSSRef", "state"],
        ["conditionalCSS", "conditionalCSSRef", "conditional"]
      ];
      ...
      function visit(node) {
        ...
        for (const [field] of fields) {
          ...
          const key = `${field}|${JSON.stringify(node[field])}`;
          counts.set(key, (counts.get(key) || 0) + 1);
        }
        if (node.pseudoElements) {
          if (node.pseudoElements.before) visit(node.pseudoElements.before);
          if (node.pseudoElements.after) visit(node.pseudoElements.after);
        }
        for (const child of node.subTreeElements || []) visit(child);
      }

      visit(rootElement);
      const definitions = {};
      const references = new Map();
      const sequence = { computed: 0, state: 0, conditional: 0 };
      for (const node of nodes) {
        for (const [field, referenceField, prefix] of fields) {
          if (!Object.hasOwn(node, field)) continue;
          const key = `${field}|${JSON.stringify(node[field])}`;
          if (counts.get(key) < 2) continue;
          if (!references.has(key)) {
            const reference = `${prefix}-${++sequence[prefix]}`;
            references.set(key, reference);
            definitions[reference] = node[field];
          }
          delete node[field];
          node[referenceField] = references.get(key);
        }
      }
      return definitions;
    }
    ...
    function cssReport(rootElement) {
      ...
      const elements = [rootElement, ...rootElement.querySelectorAll("*")].filter(isInspectableElement);
      for (const element of elements) {
        const selector = selectorFor(element, rootElement);
        blocks.push(cssBlock(selector, relevantComputedStyle(element)));
        const before = pseudoData(element, "::before");
        const after = pseudoData(element, "::after");
        if (before) blocks.push(cssBlock(`${selector}::before`, before.computedCSS));
        if (after) blocks.push(cssBlock(`${selector}::after`, after.computedCSS));
        for (const record of [
    ...
    try {
      ...
      if (!isInspectableElement(selected)) return { ok: false, code: "NON_RENDERING_ELEMENT", message: "The selected element does not represent rendered document content." };
      ...
      if (mode === "css") return {
        ok: true,
        mode,
        output: cssReport(selected),
        inaccessibleStylesheetUrls: stylesheetAccessIssues.map((issue) => issue.url).filter((url) => url !== "(inline stylesheet)")
      };
      if (mode === "json") {
        const rootElement = inspectElement(selected, selected);
        const styleDefinitions = combineStyles ? combineStyleDefinitions(rootElement) : {};
        const report = {
          _about: combineStyles
            ? "Selected HTML subtree in original parent-child order with relevant CSS. Resolve *CSSRef values through styleDefinitions."
            : "Selected HTML subtree in original parent-child order, with relevant applied, state, and conditional CSS for visual layout and debugging.",
          meta: { generator: "CSS Tree Inspector by 24Moves.com", version: VERSION, url: location.href, generatedAt: new Date().toISOString() },
          ...(Object.keys(styleDefinitions).length ? { styleDefinitions } : {}),
          rootElement,
  ...
  root.CSS_TREE_INSPECTOR_EXPRESSION = function (mode, options) {
    return `(${inspectedPageTask.toString()})(${JSON.stringify(mode)}, ${JSON.stringify(options || {})})`;
  };
})(globalThis);
