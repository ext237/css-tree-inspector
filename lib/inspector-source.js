(function (root) {
  ...
  function inspectedPageTask(mode) {
    ...
    function collectStyleRules() {
      ...
      function visit(ruleList, atRuleContext, active) {
        for (const rule of Array.from(ruleList || [])) {
          ...
          if (rule.type === CSSRule.IMPORT_RULE && rule.styleSheet) {
            ...
            try { visit(rule.styleSheet.cssRules, importContext, importActive); } catch (_) { /* Inaccessible imports are skipped. */ }
            continue;
          }
          ...
          if (typeof CSSMediaRule !== "undefined" && rule instanceof CSSMediaRule) {
            childActive = active && matchMedia(rule.conditionText).matches;
          ...
          visit(rule.cssRules, childContext, childActive);
        }
      }
      ...
      for (const sheet of sheets) {
        ...
        try { visit(sheet.cssRules, sheetContext, sheetActive); } catch (_) { /* Cross-origin stylesheets are intentionally skipped. */ }
      }
      return rules;
    }
    ...
    function cssReport(rootElement) {
      ...
      for (const element of elements) {
        ...
        for (const record of [
          ...
          if (!authoredStateRules.has(record.identity)) authoredStateRules.set(record.identity, record);
        }
      }
